...
/*
    1. Add mechanism to warn leader about intruder. Send intruder info so that
   the leader can maintain appropriate speed and consequently the whole platoon
   runs at the same speed (intruder's speed)
        **Detals: Define intruder warning message struct, message type and
   payload. Use it on both the leader and the follower.
...
int main(int argc, char *argv[]) {
  ...
  // 2. Send Registration and get Assigned ID
  join_platoon(tcp2Leader, my_ip, my_port);

  // ID will be assigned via TCP listener
  follower_idx = 0;

  // Initial position (placeholder, will be snapped by TCP listener)
  follower =
      (Truck){.x = 0, .y = 0.0f, .speed = 0, .dir = NORTH, .state = CRUISE};
...
void *tcp_listener(void *arg) {
  ...
  while (1) {
    ...
    case MSG_LDR_CMD:
      ...
      Event cmd_evt = {.type = EVT_CRUISE_CMD,
                       .event_data.leader_cmd = msg.payload.cmd};
