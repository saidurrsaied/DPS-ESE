...
pthread_t gpu_intruder_tid; //GPU
...
static void* gpu_intruder_listener(void* arg); //GPU
...
} GpuIntruderSample; //GPU

static int read_gpu_intruder(const char* path, GpuIntruderSample* out); //GPU

static uint64_t last_leader_rx_ms = 0;
static int leader_timeout_emitted = 0;
static const char* gpu_intruder_path = NULL; //GPU
...
int main(int argc, char* argv[]) {
    ...
    const char* env_intruder_path = getenv("GPU_INTRUDER_FILE"); //GPU
    gpu_intruder_path = (env_intruder_path && env_intruder_path[0]) ? env_intruder_path : GPU_INTRUDER_FILE; //GPU
    ...
    pthread_create(&gpu_intruder_tid, NULL, gpu_intruder_listener, NULL); //GPU
    ...
    pthread_join(gpu_intruder_tid, NULL); //GPU
...
static void* gpu_intruder_listener(void* arg) { //GPU_implementation
    (void)arg;
    ...
    while (!follower_shutdown_requested) {
        ...
        if (read_gpu_intruder(gpu_intruder_path, &cur) == 0) {
            if (!has_last || cur.seq != last.seq) {
                if (cur.target_id != 0 && follower_idx != 0 && cur.target_id != follower_idx) { 
                    last = cur;
                    has_last = 1;
